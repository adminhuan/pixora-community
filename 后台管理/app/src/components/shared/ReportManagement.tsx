...
import { HandleReportModal } from '../../pages/CommentAdmin/components/HandleReportModal';
import { ReportDetail } from '../../pages/CommentAdmin/components/ReportDetail';
import { ReportTable } from '../../pages/CommentAdmin/components/ReportTable';
...
const ReportManagement = ({ contentType, title = '举报管理' }: ReportManagementProps) => {
    ...
    async (preferredId?: string) => {
      ...
      try {
        const [reportRes, statsRes] = await Promise.all([
          adminCommentApi.reports({ targetType: contentType }),
          adminCommentApi.reportStats(),
        ]);
        const list = extractList<Record<string, unknown>>(reportRes).map((item) => {
          ...
          const reporterName = String(
            item.reporterName ?? getDisplayName(item.reporter, String(item.reporterId ?? '-')),
          );
